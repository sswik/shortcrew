...
class TestBridgeCurate(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self._env = mock.patch.dict(os.environ, {
            ...
            "CHANNEL_05_FILE_ID": "SHEET_X",
            "CHANNEL_05_TAB": "홈캠카오스-상품",
        })
        self._env.start()
    ...
    def test_token_required(self) -> None:
        self.assertEqual(self._post({"channel_id": "05"}, token="").status_code, 401)
    ...
    def test_no_topics_uses_trend_keywords(self) -> None:
        ...
        # 05 trend_keywords 중 '홈캠' 주제가 상품 선정됨
    ...
    def test_dry_run_no_sheet_write(self) -> None:
        with mock.patch.object(bridge.coupang, "search_products", _fake_search), \
                mock.patch.object(bridge.gemini_curator, "pick_product_for_topic", _fake_pick), \
                mock.patch.object(bridge.coupang, "generate_deeplinks", _fake_deeplinks):
            r = self._post({"channel_id": "05", "topics": ["현관 홈캠", "우주의 기원"], "dry_run": True})
        self.assertEqual(r.status_code, 200, r.text)
        d = r.json()
    ...
    def test_writes_products_to_sheet(self) -> None:
        with mock.patch.object(bridge.coupang, "search_products", _fake_search), \
            ...
            r = self._post({"channel_id": "05", "topics": ["현관 홈캠 사각지대"], "dry_run": False})
        self.assertEqual(r.status_code, 200, r.text)
        d = r.json()
    ...
    def test_sheet_missing_returns_503(self) -> None:
        with mock.patch.dict(os.environ, {"CHANNEL_05_FILE_ID": ""}), \
                mock.patch.object(bridge.coupang, "search_products", _fake_search), \
                mock.patch.object(bridge.gemini_curator, "pick_product_for_topic", _fake_pick), \
                mock.patch.object(bridge.coupang, "generate_deeplinks", _fake_deeplinks):
            r = self._post({"channel_id": "05", "topics": ["현관 홈캠"], "dry_run": False})
        self.assertEqual(r.status_code, 503)

    def test_auto_blog_creates_blogpost_with_product_image(self) -> None:
        ...
        with mock.patch.object(bridge.coupang, "search_products", _fake_search), \
                ...
                mock.patch.object(google_sheets, "get_existing_values", _fake_existing), \
                mock.patch.object(google_sheets, "append_rows", _fake_append), \
                mock.patch.object(blog_service, "generate_review_draft", _fake_review_draft):
            r = self._post({
                "channel_id": "05", "topics": ["현관 홈캠 사각지대"],
        ...
        posts = [o for o in _FakeDB.added if isinstance(o, BlogPost)]
        self.assertEqual(len(posts), 1)
        self.assertEqual(posts[0].pump_slug, "homecam")
        self.assertEqual(posts[0].product_image_url, "i1")  # 상품이미지 필수
        self.assertEqual(posts[0].status, "published")
        self.assertIn("lptag=deep", posts[0].product_deeplink)
    ...
    def test_auto_dm_creates_rule(self) -> None:
        ...
        self.assertEqual(dms[0].channel_id, "05")
