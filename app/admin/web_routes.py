"""백오피스(어드민) HTML 라우트: 로그인·대시보드·리뷰 CRUD·인플루언서·상품/DM/시트/로그.

JSON Ops API는 app/admin/ops/, 공개 라우트는 app/client/routes.py. 이 라우터는 main 에서
공개 catch-all `/{name_slug}` 보다 먼저 include 한다."""
...
@router.get("/admin/reviews/{review_id}/edit", response_class=HTMLResponse)
...
@router.get("/admin/reviews/product-options")
...
@router.post("/admin/reviews/{review_id}/edit")
...
@router.get("/admin/influencers", response_class=HTMLResponse)
...
@router.get("/admin/influencers/{name_slug}/edit", response_class=HTMLResponse)
...
@router.post("/admin/influencers/{name_slug}/edit")
...
@router.get("/admin/dm", response_class=HTMLResponse)
def admin_dm(
    request: Request,
    _: None = Depends(require_admin),
) -> HTMLResponse:
...
@router.get("/admin/sheets", response_class=HTMLResponse)
def admin_sheets(
    request: Request,
    _: None = Depends(require_admin),
) -> HTMLResponse:
