"""지서아픽(공간인플루언서) — channel_id·env 접두어 `106` — `.env` 의 `CHANNEL_106_*`. 큐레이션 인플루언서(세트)."""
...
def build() -> dict:
    ...
    return {
        ...
        "mall_influencer_slug": (os.getenv(channel_env(_CID, "MALL_INFLUENCER_SLUG"), "").strip() or "space"),
        "naver_category_id": [
            NAVER_CATEGORY_IDS["가구/인테리어"],
