...
6. `reviews`(MySQL) 에 INSERT 하며 `source_youtube_video_id` 로 **동일 영상 재발행**을 막는다.
...
호스트 cron 대신 컨테이너에서 **매일 06:00 KST**에 같은 스크립트를 돌리려면: `docker compose build && docker compose --profile shorts-cron up -d`. 상세는 [07_쇼츠자동화진단교정.md](07_쇼츠자동화진단교정.md) 7절.
...
| [`scripts/seed_influencers_from_roster.py`](../scripts/seed_influencers_from_roster.py) | 로스터 기준 `influencers` 시드(MySQL) |
