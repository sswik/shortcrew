...
> ⚠️ 이 문서가 다루는 '쇼츠 시트 자동발행(06:00 크론)' 기능은 폐기됨(2026-06). 현행은 숏크루 내부 큐레이션 스케줄러 + 유튜브/IG 크로스포스트 + 댓글→DM 퍼널. 이력 참고용으로만 보존.

[06_쇼츠시트리뷰자동화.md](06_쇼츠시트리뷰자동화.md) 파이프라인이 **기대대로 돌았는지** 확인한 뒤, 아래 순서로 운영·수정하면 된다. 앱 프로세스만 띄워 두면 **스스로 주기 실행되지 않는다**는 점이 전제다 (`scripts/run_shorts_sheet_reviews.py` 또는 어드민 API 호출이 필요).
...
| `실행할 채널이 없습니다` | 자동화 플래그 꺼짐 또는 시트 필수값 없음 | `CHANNEL_<ID>_SHORTS_AUTOMATION_ENABLED`, `SHORTS_PLAN_TAB`, `GOOGLE_SHEET_ID` 등 [06](06_쇼츠시트리뷰자동화.md) 표대로 채움 |
...
| `자막·메타 없음 스킵` | 자막 없는 영상, API/쿼터 | 영상·키 확인; `yt-dlp` 설치 권장([06](06_쇼츠시트리뷰자동화.md)) |
...
2. **Docker Compose에 전용 서비스**  
   ...
   - `google-key.json`·`.env`를 워커에도 동일 마운트(DB는 MySQL `mysql-server:3306` 공유, `ejlab_global_net`).
...
| 상세 스펙 | [06_쇼츠시트리뷰자동화.md](06_쇼츠시트리뷰자동화.md) |
...
1. **스케줄 등록**  
   - 이 프로젝트 루트에서 `crontab -e` 등으로 [06](06_쇼츠시트리뷰자동화.md)의 `run_shorts_sheet_reviews.py` **cron 예시 한 줄**을 넣는다(절대 경로·`.venv/bin/python`·`TZ=Asia/Seoul` 확인).  
...
`shortcrew` 서비스와 **동일** `build` / `env_file` / `google-key.json` 볼륨 / `ejlab_global_net` 네트워크를 쓰고, 프로필만 분리한다. DB 는 MySQL(`mysql-server:3306`)을 공유한다.
...
- **동시성**: 웹(`shortcrew`)과 cron 컨테이너가 같은 MySQL(`crews`)에 접속한다. MySQL 은 동시 쓰기를 처리하므로 과거 SQLite 파일 락 이슈는 없다.

---

## 8. 실행 방법 (복붙용)

프로젝트 **루트**에서 실행한다. `.env`·`google-key.json` 이 그 루트에 있어야 한다(DB는 `.env` 의 `DATABASE_URL` MySQL).
...
- **[docker-compose.yml](../docker-compose.yml)**  
  ...
  - **`env_file: .env`**, **`google-key.json` 볼륨**, **`ejlab_global_net`** 은 웹과 동일 — DB는 MySQL `mysql-server:3306` 을 공유한다.

- **문서**  
  - 본 07(실행·정리), [06](06_쇼츠시트리뷰자동화.md)(Compose 한 절).
...
3. 그 스크립트는 **MySQL(`crews`)** 에 접속하고, `.env` 의 키·`CHANNEL_*` 로 시트를 읽어 [06](06_쇼츠시트리뷰자동화.md)과 **동일한 파이프라인**(매칭 → 자막 → Gemini → `reviews` INSERT)을 탄다.  
4. 웹 서비스 `shortcrew` 과는 **프로세스가 분리**되어 있고, **같은 MySQL DB**(`crews`)를 공유한다 — MySQL 이 동시 접근을 처리한다.
...
flowchart LR
  ...
  subgraph dbnet [ejlab_global_net]
    db["MySQL mysql-server:3306 (crews)"]
  end
