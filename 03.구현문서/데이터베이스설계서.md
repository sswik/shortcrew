...
> 엔진: SQLAlchemy 2.0 + MySQL(`mysql-server:3306`, DB `crews`). 접속은 `.env` 의 `DATABASE_URL` 필수.
...
- 제약: `UNIQUE(influencer_slug, source_youtube_video_id)` — 영상 출처(쇼츠/유튜브) 기준 중복 발행 방지.
...
- `scripts/seed_influencers_from_roster.py` — 채널 로스터(golf/tennis/shopping) 기준으로 초기 인플루언서 데이터 셋업(MySQL).
- 스키마는 `Base.metadata.create_all` 로 생성(Alembic 미사용).
